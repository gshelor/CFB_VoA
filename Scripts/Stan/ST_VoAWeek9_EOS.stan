...
data {
  ...
  vector[N] net_kick_return_avg; // yards per kick return
  vector[N] net_punt_return_avg; // yards per punt return
  vector[N] net_fg_rate; // field goal conversion rate
  vector[N] net_fg_made_pg; // number of field goals made per game
  vector[N] net_xpts_pg; // extra points per game
}
...
parameters {
  ...
  real beta_net_xpts_pg; // coefficient for extra points per game
...
model {
  ...
  net_st_ppg ~ normal(b0 + beta_net_kick_return_avg * net_kick_return_avg + beta_net_punt_return_avg * net_punt_return_avg + beta_net_fg_rate * net_fg_rate + beta_net_fg_made_pg * net_fg_made_pg + beta_net_xpts_pg * net_xpts_pg, sigma);
}
