...
data {
  ...
  vector[N] def_ppa_PY3; // EPA per play
  vector[N] def_ppa_PY2; // EPA per play
  vector[N] def_ppa_PY1; // EPA per play
  vector[N] def_ypp_PY3; // Yards per play
  vector[N] def_ypp_PY2; // Yards per play
  vector[N] def_ypp_PY1; // Yards per play
  vector[N] def_success_rate_PY3; // Play success rate
  vector[N] def_success_rate_PY2; // Play success rate
  vector[N] def_success_rate_PY1; // Play success rate
  vector[N] def_explosiveness_PY3; // defensive explosiveness rate
  vector[N] def_explosiveness_PY2; // defensive explosiveness rate
  vector[N] def_explosiveness_PY1; // defensive explosiveness rate
  vector[N] def_third_conv_rate_PY3; // third down conversion rate
  vector[N] def_third_conv_rate_PY2; // third down conversion rate
  vector[N] def_third_conv_rate_PY1; // third down conversion rate
  vector[N] def_pts_per_opp_PY3; // points per scoring opportunity
  vector[N] def_pts_per_opp_PY2; // points per scoring opportunity
  vector[N] def_pts_per_opp_PY1; // points per scoring opportunity
  vector[N] def_plays_pg_PY1; // number of defensive plays per game
...
parameters {
  real b0; // intercept
  real beta_def_ppa_PY3; // Coefficient for EPA
  real beta_def_ppa_PY2; // Coefficient for EPA
  real beta_def_ppa_PY1; // Coefficient for EPA
  real beta_def_ypp_PY3; // Coefficient for yards per play
  real beta_def_ypp_PY2; // Coefficient for yards per play
  real beta_def_ypp_PY1; // Coefficient for yards per play
  real beta_def_success_rate_PY3; // Coefficient for def success rate
  real beta_def_success_rate_PY2; // Coefficient for def success rate
  real beta_def_success_rate_PY1; // Coefficient for def success rate
  real beta_def_explosiveness_PY3; // Coefficient for def explosiveness rate
  real beta_def_explosiveness_PY2; // Coefficient for def explosiveness rate
  real beta_def_explosiveness_PY1; // Coefficient for def explosiveness rate
  real beta_def_third_conv_rate_PY3; // coefficient for third down conversion rate
  real beta_def_third_conv_rate_PY2; // coefficient for third down conversion rate
  real beta_def_third_conv_rate_PY1; // coefficient for third down conversion rate
  real beta_def_pts_per_opp_PY3; // coefficient for defensive points per scoring opportunity
  real beta_def_pts_per_opp_PY2; // coefficient for defensive points per scoring opportunity
  real beta_def_pts_per_opp_PY1; // coefficient for defensive points per scoring opportunity
  real beta_def_plays_pg_PY1; // coefficient for number of plays run by the defense
...
model {
  ...
  def_ppg ~ normal(b0 + beta_def_ppa_PY3 * def_ppa_PY3 + beta_def_ppa_PY2 * def_ppa_PY2 + beta_def_ppa_PY1 * def_ppa_PY1 + beta_def_ypp_PY3 * def_ypp_PY3 + beta_def_ypp_PY2 * def_ypp_PY2 + beta_def_ypp_PY1 * def_ypp_PY1 + beta_def_success_rate_PY3 * def_success_rate_PY3 + beta_def_success_rate_PY2 * def_success_rate_PY2 + beta_def_success_rate_PY1 * def_success_rate_PY1 + beta_def_explosiveness_PY3 * def_explosiveness_PY3 + beta_def_explosiveness_PY2 * def_explosiveness_PY2 + beta_def_explosiveness_PY1 * def_explosiveness_PY1 + beta_def_third_conv_rate_PY3 * def_third_conv_rate_PY3 + beta_def_third_conv_rate_PY2 * def_third_conv_rate_PY2 + beta_def_third_conv_rate_PY1 * def_third_conv_rate_PY1 + beta_def_pts_per_opp_PY3 * def_pts_per_opp_PY3 + beta_def_pts_per_opp_PY2 * def_pts_per_opp_PY2 + beta_def_pts_per_opp_PY1 * def_pts_per_opp_PY1 + beta_def_plays_pg_PY1 * def_plays_pg_PY1 + beta_def_ppg_aboveavg * def_ppg_aboveavg + beta_VoA_Output * VoA_Output + beta_Conference_Strength * Conference_Strength + beta_def_ppa * def_ppa + beta_def_ypp * def_ypp + beta_def_success_rate * def_success_rate + beta_def_explosiveness * def_explosiveness + beta_def_third_conv_rate * def_third_conv_rate + beta_def_pts_per_opp * def_pts_per_opp, sigma);
}
