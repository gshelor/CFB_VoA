...
model {
  ...
  def_ppg ~ normal(b0 + beta_def_ppa_PY2 * def_ppa_PY2 + beta_def_ppa_PY1 * def_ppa_PY1 + beta_def_ypp_PY2 * def_ypp_PY2 + beta_def_ypp_PY1 * def_ypp_PY1 + beta_def_success_rate_PY2 * def_success_rate_PY2 + beta_def_success_rate_PY1 * def_success_rate_PY1 + beta_def_explosiveness_PY2 * def_explosiveness_PY2 + beta_def_explosiveness_PY1 * def_explosiveness_PY1 + beta_def_third_conv_rate_PY2 * def_third_conv_rate_PY2 + beta_def_third_conv_rate_PY1 * def_third_conv_rate_PY1 + beta_def_pts_per_opp_PY2 * def_pts_per_opp_PY2 + beta_def_pts_per_opp_PY1 * def_pts_per_opp_PY1 + beta_def_plays_pg * def_plays_pg + beta_def_ppg_aboveavg * def_ppg_aboveavg + beta_VoA_Output * VoA_Output + beta_Conference_Strength * Conference_Strength + beta_def_ppa * def_ppa + beta_def_ypp * def_ypp + beta_def_success_rate * def_success_rate + beta_def_explosiveness * def_explosiveness + beta_def_third_conv_rate * def_third_conv_rate + beta_def_pts_per_opp * def_pts_per_opp, sigma);
}
