...
upcoming_games_df <- upcoming_games_df |>
  mutate(Proj_Winner = case_when((neutral_site == FALSE & (home_VoA_Rating + 2) > away_VoA_Rating) ~ home_team, (neutral_site == FALSE & away_VoA_Rating > (home_VoA_Rating + 2)) ~ away_team, (neutral_site == TRUE & home_VoA_Rating > away_VoA_Rating) ~ home_team, (neutral_site == TRUE & away_VoA_Rating > home_VoA_Rating) ~ away_team, TRUE ~ "TIE"),Proj_Margin = case_when(neutral_site == FALSE ~ abs(away_VoA_Rating - (home_VoA_Rating + 2)), TRUE ~ abs(away_VoA_Rating - home_VoA_Rating)), Initial_Win_Prob = 50.37036489 + (2.38892221 * Proj_Margin) + (-0.02809534 * (Proj_Margin^2))) |>
